...
3. **インラインコメントの書き方**
   - コメントは「何をしているか」だけでなく「**なぜそうしているか**・これがないとどうなるか」まで書く。
   - `NOTE:` プレフィックスを使い、背景・影響・参考リンクをセットで記載する。
     ```toml
     # NOTE: "$auto" にすると dpkg-shlibdeps が arm64 ライブラリを x86_64 ホストで解決できず失敗する。
     #       そのため libc6 を直接指定している。
     #       参考: <失敗した CI ログの URL など、根拠となる具体的なリンク>
     ```
   - 参考リンクは**根拠のあるもの**だけ貼る。根拠のない URL を推測して貼ってはいけない。
     失敗した CI ログ・issue・PR など、手元にある具体的な証拠を使うこと。

4. **PR 説明文はコミットのたびに見直す**
   - リファクタリングで実装方針が変わった場合、PR 説明の「変更内容」「修正の詳細」も必ず更新する。
   - `gh pr edit --body` で PR 説明を上書きする。説明が古いままだとレビュアーに誤解を与える。

5. **ローカルビルドへの影響を考慮した CI 修正**
   - `.cargo/config.toml` などのプロジェクト設定ファイルは、CI 専用の workaround を書くと
     ローカル開発環境にも影響する。CI 内だけで完結できる方法（インストールステップ追加・
     フラグ変更など）を優先し、プロジェクト設定ファイルは変更しない。
