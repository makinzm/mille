...
- **コミット順序**: テスト失敗確認（`--no-verify`）→ 実装して全テスト通過 → リファクタ
  - `[test] <内容> because of <理由>` / `[fix] ...` / `[refactor] ...`
- **TDD は全対象に対して行う**: 複数の対象（例: 8言語パーサー）に同じ変更を加える場合、**全対象のテストを先に書く**。「同じパターンだから」でテストをスキップしない
...
**Rust TDD の手順:**
1. **テスト失敗確認**: 全対象のテストを書き、テストが失敗する（またはコンパイルエラーになる）ことを確認する。`--no-verify` でコミット
2. **実装して全テスト通過**: 実装を追加し、全テストが通過することを確認する → lefthook 通過でコミット
3. **リファクタ**: 必要なら整理
