...
### PR 6: メソッド呼び出しチェック（`allow_call_patterns`）機能の実装
- [ ] Rust用 tree-sitter でのメソッド呼び出し（`call_expression`）構文抽出ロジックの追加
- [ ] 依存ルールエンジンに `allow_call_patterns` 制約への違反検出機能を追加
- [ ] `Violation` / `ViolationKind` に `CallPatternViolation` を追加

### PR 7: TypeScript / JavaScript サポートの追加
...
### PR 9: 外部ライブラリ依存（`external_mode`）チェックの実装
