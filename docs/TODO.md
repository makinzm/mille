...
- ✅ `mille add` コマンド — 既存の `mille.toml` にディレクトリをレイヤーとして追加。ターゲットをスキャンして `[[layers]]` を追記、重複時は `--force` で置換。`--name` でレイヤー名カスタマイズ可（PR #86）
- ✅ `mille add --depth` — サブディレクトリを個別レイヤーとして複数追加。`mille init` と同じ depth ベースのレイヤー検出を再利用（PR #87）
...
- [x] Kotlin サポート（別 PR）— `infrastructure::parser::kotlin` 実装 (tree-sitter-kotlin)、flat/Gradle レイアウト、E2E テスト

### PHP サポート ✅ 完了

- [x] `infrastructure::parser::php` 実装 (tree-sitter-php 0.22) — use 文 (simple/aliased/grouped/function/const)・class/function 名・コメントのパース
- [x] `infrastructure::resolver::php` 実装 — Internal/External/Stdlib 分類、`composer.json` PSR-4 自動検出
- [x] `[resolve.php]` 設定セクション (`namespace` / `composer_json`) の追加
- [x] `ext_to_language` に `.php` 追加（`mille init` で PHP プロジェクト自動検出）
- [x] README.md 更新（言語サポート表・`[resolve.php]` リファレンス・Laravel 設定例）
