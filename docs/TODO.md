...
### PR N+2: packages/go の external_allow 修正（mille.toml による依存チェック機能化）
- 背景: 現在 `packages/go/mille.toml` の `external_allow` が雑な設定になっており、
  依存ルールチェックが実質的に機能していない。
  mille 自身が Go ラッパーの依存を正しく検査できる状態にする（Dogfooding の精度向上）。
- [ ] `packages/go/mille.toml` の `external_allow` を wazero・packages/wasm のみに絞り込む
- [ ] `go test ./...` の `TestRunWasm_SelfCheck` が violation 0 件で通ることを確認
- [ ] CI `dogfood-go` の self-check ステップも violation 0 件で通ることを確認
