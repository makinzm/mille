...
| ファイル | 役割 |
|----------|------|
| `packages/go/mille.wasm` | **唯一のコミット済み .wasm**。`packages/go/main.go` が `//go:embed` で直接埋め込む |
| `packages/npm/mille.wasm` | publish 時にのみ生成（`packages/go/mille.wasm` からコピー）。リポジトリにはコミットしない |
...
| mille.wasm | `packages/go/mille.wasm`（リポジトリにコミット済み） |

### Step 1: mille.wasm を npm パッケージにコピー

```bash
# リポジトリ root で実行
cp packages/go/mille.wasm packages/npm/mille.wasm
```
...
**出力ファイル:**
- `packages/go/mille.wasm` — 唯一の canonical Wasm バイナリ（Git 管理）
...
`packages/go/main.go` が `//go:embed mille.wasm` で直接埋め込みます。
外部モジュール依存はありません。
...
| `update-wasm` | artifact の wasm で `packages/go/mille.wasm` を上書きコミット | **main push のみ** |
...
git add packages/go/mille.wasm
...
`packages/go/mille.wasm`（約 3MB）は Git で追跡されています。
