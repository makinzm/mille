...
- [ ] `packages/go/go.mod` に replace ディレクティブ追加（local dev 用）
- [ ] `git rm --cached packages/go/mille.wasm`（git 追跡解除）
- [ ] `.gitignore` に `packages/go/mille.wasm` 追加
- [ ] `scripts/build-wasm.sh` を packages/wasm のみコピーに修正
- [ ] `go mod tidy` 実行（go.sum 更新）
- [ ] `go test ./...` パス確認
- [ ] CI `build-wasm` ジョブ更新（packages/go/mille.wasm への参照を削除）
- [ ] `docs/administrator/wasm_build.md` 更新（single copy・go.work 説明）
- [ ] リファクタリングコミット
