...
//! where each language has its own directory subtree (ts/, py/, go/).
//!
//! Fixture: `tests/fixtures/multilang_split_sample/`
//! Languages: TypeScript + Python + Go (separated into ts/, py/, go/ directories)
...
fn fixture_dir() -> PathBuf {
    project_root().join("tests/fixtures/multilang_split_sample")
}
...
// ---------------------------------------------------------------------------
// 1. Happy path: valid multi-language split fixture
// ---------------------------------------------------------------------------

#[test]
fn test_multilang_split_valid_config_exits_zero() {
    let out = mille_in_fixture(&["check"]);
    assert_eq!(
        exit_code(&out),
        0,
        "multilang_split_sample should produce no violations\nstdout:\n{}\nstderr:\n{}",
        stdout(&out),
        stderr(&out)
    );
}
...
#[test]
fn test_multilang_split_valid_config_summary_shows_zero_errors() {
    let out = mille_in_fixture(&["check"]);
    let s = stdout(&out);
    assert!(
...
const SPLIT_BROKEN_DEP_OPTIN_TOML: &str = r#"
[project]
name = "multilang-split"
root = "."
languages = ["typescript", "python", "go"]

[resolve.go]
module_name = "github.com/example/multilang-split"
...
[[layers]]
name = "domain"
paths = ["ts/domain/**", "py/domain/**", "go/domain/**"]
...
[[layers]]
name = "usecase"
paths = ["ts/usecase/**", "py/usecase/**", "go/usecase/**"]
dependency_mode = "opt-in"
allow = []
...
[[layers]]
name = "infrastructure"
paths = ["ts/infrastructure/**", "py/infrastructure/**", "go/infrastructure/**"]
...
#[test]
fn test_multilang_split_broken_dep_optin_exits_one() {
    let dir = fixture_dir();
    let config = dir.join("mille_broken_dep_optin.toml");
    std::fs::write(&config, SPLIT_BROKEN_DEP_OPTIN_TOML).unwrap();
...
#[test]
fn test_multilang_split_broken_dep_optin_mentions_usecase() {
    let dir = fixture_dir();
...
const SPLIT_BROKEN_DEP_OPTOUT_TOML: &str = r#"
[project]
name = "multilang-split"
root = "."
languages = ["typescript", "python", "go"]
...
#[test]
fn test_multilang_split_broken_dep_optout_exits_one() {
    let dir = fixture_dir();
    let config = dir.join("mille_broken_dep_optout.toml");
    std::fs::write(&config, SPLIT_BROKEN_DEP_OPTOUT_TOML).unwrap();
    let out = Command::new(env!("CARGO_BIN_EXE_mille"))
...
#[test]
fn test_multilang_split_broken_dep_optout_mentions_infrastructure() {
    let dir = fixture_dir();
...
const SPLIT_BROKEN_EXT_OPTIN_TOML: &str = r#"
[project]
name = "multilang-split"
root = "."
languages = ["typescript", "python", "go"]
...
#[test]
fn test_multilang_split_broken_external_optin_exits_one() {
    let dir = fixture_dir();
    let config = dir.join("mille_broken_ext_optin.toml");
    std::fs::write(&config, SPLIT_BROKEN_EXT_OPTIN_TOML).unwrap();
    let out = Command::new(env!("CARGO_BIN_EXE_mille"))
...
#[test]
fn test_multilang_split_broken_external_optin_mentions_domain() {
    let dir = fixture_dir();
...
const SPLIT_BROKEN_EXT_OPTOUT_TOML: &str = r#"
[project]
name = "multilang-split"
root = "."
languages = ["typescript", "python", "go"]
...
#[test]
fn test_multilang_split_broken_external_optout_exits_one() {
    let dir = fixture_dir();
    let config = dir.join("mille_broken_ext_optout.toml");
    std::fs::write(&config, SPLIT_BROKEN_EXT_OPTOUT_TOML).unwrap();
    let out = Command::new(env!("CARGO_BIN_EXE_mille"))
...
#[test]
fn test_multilang_split_broken_external_optout_mentions_infrastructure() {
    let dir = fixture_dir();
...
const SPLIT_BROKEN_CALL_PATTERN_TOML: &str = r#"
[project]
name = "multilang-split"
root = "."
languages = ["typescript", "python", "go"]
...
#[test]
fn test_multilang_split_broken_call_pattern_exits_one() {
    let dir = fixture_dir();
    let config = dir.join("mille_broken_call_pattern.toml");
    std::fs::write(&config, SPLIT_BROKEN_CALL_PATTERN_TOML).unwrap();
    let out = Command::new(env!("CARGO_BIN_EXE_mille"))
